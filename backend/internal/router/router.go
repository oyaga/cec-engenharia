...
func New(cfg *config.Config, gdb *gorm.DB, tm *auth.TokenManager, cch *cache.Cache) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		u := v1.Group("/users")
		u.Use(middleware.RequireAuth(tm))
		userManagers := middleware.RequireRole("admin", "coordenador", "webdesigner")
		u.GET("", userManagers, usersH.List)
		u.POST("", userManagers, usersH.Create)
		u.PUT("/:id", userManagers, usersH.Update)
		u.DELETE("/:id", userManagers, usersH.Delete)
		adminUserManagers := middleware.RequireRole("admin", "coordenador")
		u.POST("/link-student/:studentId", adminUserManagers, usersH.LinkStudentAccount)
		u.GET("/:id", adminUserManagers, usersH.Get)
		u.POST("/:id/reset-password", adminUserManagers, usersH.ResetPassword)
		u.POST("/:id/send-password-reset", adminUserManagers, usersH.SendPasswordReset)
		...
		att.POST("", staffOnly, acadH.CreateAttendance)
		att.PUT("/:id", staffOnly, acadH.UpdateAttendance)
		...
		// Revisão de documentos — apenas staff (admin/coordenador/atendente)
		st.POST("/:id/review-doc", staffOnly, studentsH.ReviewDoc)
